...
function gdbm_open(name::String, flag::String="r")
  ...
  handle == C_NULL && error("Le fichier ne peut être ouvert!")
  handle
end
...
function gdbm_store(handle::Ptr{Cvoid}, key::Datum, value::Datum, flag::String="r")
  ...
  ret == -1 && error("Impossible d'écrire dans la base de données ou les clés/valeurs sont des chaînes non valides.")
  ret == 1 && error("La clé est déjà dans la base de données.")
  nothing
end
...
function gdbm_count(handle::Ptr{Cvoid})
  ...
  ret == -1 && error("Erreur de lecture de la base de données.")
  Int(count[])
end
...
function gdbm_delete(handle::Ptr{Cvoid}, key::Datum)
  ...
  ret ≠ 0 && error("Impossible d'écrire dans la base de données ou clé introuvable.")
  nothing
end
